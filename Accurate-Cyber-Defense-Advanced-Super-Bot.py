#!/usr/bin/env python3
"""
Comprehensive Cybersecurity Monitoring Tool with Telegram Integration
Green Theme - Multi-functional Security Platform
"""
...
class Config:
    ...
    def set_telegram_config(self, token: str, chat_id: str) -> bool:
        ...
        try:
            ...
            self.config['telegram']['enabled'] = bool(token and chat_id)
...
class DatabaseManager:
    ...
    def log_telegram_alert(self, chat_id: str, message: str, status: str = "sent"):
        """Log Telegram alert"""
        query = "INSERT INTO telegram_alerts (chat_id, message, status) VALUES (?, ?, ?)"
        self.execute_query(query, (chat_id, message, status))
...
class SecurityScanner:
    ...
    def generate_recommendations(self, scan_results: Dict) -> List[str]:
        ...
        if any(port in [21, 23, 25] for port in open_ports):  # FTP, Telnet, SMTP
            recommendations.append("Consider replacing insecure protocols (FTP, Telnet) with secure alternatives")
        
        return recommendations

# Monitoring system with Telegram alerts
class MonitoringSystem:
    """Continuous monitoring system with Telegram alerts"""
    
    def __init__(self, config: Config, db: DatabaseManager, telegram_bot: 'TelegramBot'):
        self.config = config
        self.db = db
    ...
    def add_target(self, target: str, target_type: str = "ipv4") -> bool:
        ...
        try:
            ...
            if self.telegram_bot.is_configured():
                message = f"🔔 New monitoring target added:\nTarget: {target}\nType: {target_type}"
    ...
    def remove_target(self, target: str) -> bool:
        ...
        try:
            ...
            # Send Telegram notification
            if self.telegram_bot.is_configured():
                message = f"🔔 Monitoring target removed:\nTarget: {target}"
                self.telegram_bot.send_alert(message)
    ...
    def stop_monitoring(self) -> bool:
        ...
        if was_running and self.monitoring_thread:
            ...
            for target, info in list(self.monitoring_targets.items()):
                ...
                try:
                    ...
                    if old_status != status and old_status != "unknown":
                        alert_message = f"🚨 Status change detected!\nTarget: {target}\nFrom: {old_status} → To: {status}"
                        if self.telegram_bot.is_configured():
                            self.telegram_bot.send_alert(alert_message)
                        print(f"📢 {alert_message}")
                    
                    # Alert for slow response
                    if (ping_result["success"] and info["response_times"] and 
                        len(info["response_times"]) > 5):
                        avg_time = sum(info["response_times"]) / len(info["response_times"])
                        if response_time > avg_time * 2:  # Significant slowdown
                            slow_alert = f"🐌 Slow response for {target}\nCurrent: {response_time:.2f}ms, Avg: {avg_time:.2f}ms"
                            if self.telegram_bot.is_configured():
                                self.telegram_bot.send_alert(slow_alert)
...
class TelegramBot:
    """Telegram bot integration with configuration support"""
    ...
    def setup_bot(self):
        ...
        if not token or not self.is_configured():
            print("ℹ️  Telegram not configured. Use 'config_telegram' command to set up.")
            return
    ...
    def _setup_handlers(self):
        ...
        self.application.add_handler(CommandHandler("targets", self._cmd_targets))
    
    async def _cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /start command"""
        chat_id = update.effective_chat.id
        await update.message.reply_text(
            f"🔒 Cybersecurity Monitoring Bot Started!\n"
            f"Chat ID: {chat_id}\n"
...
/targets - List monitoring targets
...
*More commands available in the CLI version*
    ...
    async def _cmd_ping(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /ping command"""
        if not context.args:
            await update.message.reply_text("Usage: /ping <IP_ADDRESS>")
            return
    ...
    async def _cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        status_text = "📊 Bot is running and monitoring targets\nUse CLI for detailed status"
        await update.message.reply_text(status_text)
    
    async def _cmd_scan(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /scan command"""
        if not context.args:
            await update.message.reply_text("Usage: /scan <IP_ADDRESS>")
            return
    ...
    async def _cmd_targets(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle /targets command"""
        # This would need access to monitoring system, simplified for now
        await update.message.reply_text("Targets list available in CLI version")
    
    def start_bot(self):
        """Start the Telegram bot"""
        if self.application and self.is_configured():
            try:
            ...
            print("ℹ️  Telegram bot not started - not configured")
    ...
    def send_alert(self, message: str) -> bool:
        ...
        try:
            ...
            if self.bot:
                ...
                self.db.log_telegram_alert(chat_id, message, "sent")
                return True
        except Exception as e:
            print(f"❌ Telegram send error: {e}")
            self.db.log_telegram_alert(chat_id, message, "failed")
    ...
    def test_connection(self) -> str:
        ...
        try:
            ...
            bot.get_me()  # Test API call
            return "✅ Telegram connection successful"
        except Exception as e:
            return f"❌ Telegram connection failed: {e}"

# Main application with enhanced Telegram configuration
class CyberSecurityTool:
    ...
    def __init__(self):
        ...
        self.monitor = MonitoringSystem(self.config, self.db, self.telegram_bot)
...
╔════════════════════════════════════════════════════════════════╗
║                                                                      
║    Accurate Cyber Defense Advanced Super Bot                   
║                                                                
...
{reset_color}
    ...
    def print_prompt(self):
        ...
        primary_color = theme.get('primary_color', '\033[92m')
        reset_color = theme.get('reset_color', '\033[0m')
        prompt = f"{primary_color}Accurate>{reset_color} "
        return input(prompt)
    ...
    def execute_command(self, command: str, source: str = "cli") -> str:
        ...
        try:
            if cmd == "help":
                ...
                return self.cmd_config_telegram(args)
            elif cmd == "test_telegram":
                return self.cmd_test_telegram()
            elif cmd == "telegram_status":
                return self.cmd_telegram_status()
            elif cmd == "exit":
                self.running = False
                return "Goodbye!"
            else:
...
Example Telegram setup:
  config_telegram 123456789:ABCdefGhIjKlMnOpQrStUvWxYz 987654321
        """
        return help_text
    
    def cmd_config_telegram(self, args: List[str]) -> str:
        """Configure Telegram token and chat ID"""
        if len(args) < 2:
            return "Usage: config_telegram <bot_token> <chat_id>\nExample: config_telegram 123456789:ABCdefGhIjKlMnOpQrStUvWxYz 987654321"
        ...
        if not token or not chat_id:
            return "❌ Token and chat ID cannot be empty"
        
        if ":" not in token:
            return "❌ Invalid token format. Should be like: 123456789:ABCdefGhIjKlMnOpQrStUvWxYz"
        
        if not chat_id.lstrip('-').isdigit():
            return "❌ Invalid chat ID. Should be a numeric value"
        ...
        # Save configuration
        if self.config.set_telegram_config(token, chat_id):
            # Reinitialize Telegram bot with new config
            self.telegram_bot = TelegramBot(self.config, self.db)
            
            # Send test message
            if self.telegram_bot.send_alert("✅ Telegram configuration successful!"):
                return f"✅ Telegram configured successfully!\nToken: {token[:10]}...\nChat ID: {chat_id}"
            else:
                return "✅ Configuration saved but test message failed"
        else:
    ...
    def cmd_telegram_status(self) -> str:
        """Show Telegram status"""
        telegram_config = self.config.get_telegram_config()
        
        status_text = "🤖 Telegram Status:\n"
        status_text += f"Configured: {'✅ Yes' if self.telegram_bot.is_configured() else '❌ No'}\n"
        
        if telegram_config.get('token'):
            status_text += f"Token: {telegram_config['token'][:10]}...\n"
        else:
            status_text += "Token: Not set\n"
        
        if telegram_config.get('chat_id'):
            status_text += f"Chat ID: {telegram_config['chat_id']}\n"
        else:
            status_text += "Chat ID: Not set\n"
        
        status_text += f"Enabled: {'✅ Yes' if telegram_config.get('enabled') else '❌ No'}\n"
    ...
    def cmd_ping(self, args: List[str]) -> str:
        ...
        if result["success"]:
            ...
            if self.telegram_bot.is_configured():
                alert_msg = f"🏓 Ping successful: {target}\nTime: {result.get('response_time', 'N/A')}ms"
    ...
    def cmd_scan(self, args: List[str]) -> str:
        ...
        # Send Telegram alert if significant findings
        if open_ports > 5 and self.telegram_bot.is_configured():
            alert_msg = f"🔍 Scan completed: {target}\nOpen ports: {open_ports}"
            self.telegram_bot.send_alert(alert_msg)
        
        return response
    
    # Other command implementations remain similar but enhanced with Telegram alerts
    ...
    def cmd_deepscan(self, args: List[str]) -> str:
        ...
        if results["vulnerabilities"]:
            ...
            # Send Telegram alert for vulnerabilities
            if self.telegram_bot.is_configured():
                alert_msg = f"⚠️  Vulnerabilities found on {target}\nCount: {len(results['vulnerabilities'])}"
                self.telegram_bot.send_alert(alert_msg)
        else:
    ...
    def run_cli(self):
        ...
        if telegram_config.get('enabled'):
            ...
            print("   Use 'config_telegram <token> <chat_id>' to set up")
        ...
        # Load existing monitoring targets
